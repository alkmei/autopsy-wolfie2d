...
export default class Spawning extends WaveLevelState {
  ...
  onEnter(options: Record<string, any>): void {
    this.spawnCD = new Timer(
      1000 / this.waveInfo[this.level.currentWave].spawnRate,
    );
  ...
  update(deltaT: number): void {
    ...
    if (this.spawnCD.isStopped) {
      ...
      let chosenEnemy =
        this.enemyOp[Math.floor(Math.random() * this.enemyOp.length)];
      ...
      while (!validPos) {
        if (chosenEnemy === EnemyType.ZOMBIE) {
          if (
            ...
            this.level.tilemap.getTileAtWorldPosition(
              new Vec2(
                spawnPos.x + SpriteSizes.ZOMBIE.x,
                spawnPos.y + SpriteSizes.ZOMBIE.y,
              ),
            ) !== 0 ||
            this.level.tilemap.getTileAtWorldPosition(
              new Vec2(
            ...
            this.level.tilemap.getTileAtWorldPosition(
              new Vec2(spawnPos.x, spawnPos.y + SpriteSizes.ZOMBIE.y),
            ) !== 0
          ) {
