...
export default class ResourceManager {
  ...
  private keepResource(key: string, type: ResourceType): void {
    ...
    for (let i = 0; i < this.resourcesToUnload.length; i++) {
      ...
      if (resource.key === key && resource.resourceType === type) {
        console.log(
          "Found resource " + key + " of type " + type + ". Keeping.",
        );
  ...
  private loadTilemap(
    key: string,
    pathToTilemapJSON: string,
    callbackIfLast: Function,
  ): void {
      ...
      for (let tileset of tilemapObject.tilesets) {
        if (tileset.image) {
          ...
          resource.addDependency(
            new ResourceReference(key, ResourceType.IMAGE),
          );
  ...
  private loadSpritesheet(
    ...
    this.loadTextFile(pathToSpritesheetJSON, (fileText: string) => {
      ...
      let path =
        StringUtils.getPathFromFilePath(pathToSpritesheetJSON) +
      ...
      resource.addDependency(
        new ResourceReference(spritesheet.name, ResourceType.IMAGE),
      );
      this.resourcesToUnload.push(resource);
    ...
    request.onload = () => {
      audioCtx.decodeAudioData(
        ...
        buffer => {
          ...
          this.resourcesToUnload.push(
            new ResourceReference(key, ResourceType.AUDIO),
          );
  ...
  public loadObject(key: string, path: string, callbackIfLast: Function): void {
    this.loadTextFile(path, (fileText: string) => {
      ...
      this.resourcesToUnload.push(
        new ResourceReference(key, ResourceType.JSON),
      );
  ...
  public shader(
    key: string,
    vShaderFilepath: string,
    fShaderFilepath: string,
  ): void {
  ...
  private gl_LoadShader(
    ...
    this.loadTextFile(vpath, (vFileText: string) => {
      ...
      this.loadTextFile(fpath, (fFileText: string) => {
        ...
        const [shaderProgram, vertexShader, fragmentShader] =
          this.createShaderProgram(vShader, fShader);
        ...
        this.resourcesToUnload.push(
          new ResourceReference(key, ResourceType.SHADER),
        );
  ...
  private gl_FinishLoadingShader(callback: Function): void {
    ...
    if (
      this.loadonly_gl_ShaderProgramsLoaded ===
      this.loadonly_gl_ShaderProgramsToLoad
    ) {
  ...
  private getLoadPercent(): number {
    return (
      (this.loadonly_tilemapsLoaded / this.loadonly_tilemapsToLoad +
