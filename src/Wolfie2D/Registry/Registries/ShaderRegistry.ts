...
export default class ShaderRegistry extends Registry<ShaderType> {
  ...
  public preload() {
    ...
    this.registerAndPreloadItem(
      ShaderRegistry.POINT_SHADER,
      PointShaderType,
      "builtin/shaders/point.vshader",
      "builtin/shaders/point.fshader",
    );

    // Queue a load for the rect shader
    this.registerAndPreloadItem(
      ShaderRegistry.RECT_SHADER,
      RectShaderType,
      "builtin/shaders/rect.vshader",
      "builtin/shaders/rect.fshader",
    );

    // Queue a load for the sprite shader
    this.registerAndPreloadItem(
      ShaderRegistry.SPRITE_SHADER,
      SpriteShaderType,
      "builtin/shaders/sprite.vshader",
      "builtin/shaders/sprite.fshader",
    );

    // Queue a load for the label shader
    this.registerAndPreloadItem(
      ShaderRegistry.LABEL_SHADER,
      LabelShaderType,
      "builtin/shaders/label.vshader",
      "builtin/shaders/label.fshader",
    );
    ...
    for (let item of this.registryItems) {
      ...
      if (item.preload !== undefined) {
        rm.shader(
          item.key,
          item.preload.vshaderLocation,
          item.preload.fshaderLocation,
        );
      }
    }
  }
