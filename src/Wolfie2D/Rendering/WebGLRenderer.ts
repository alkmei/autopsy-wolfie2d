...
export default class WebGLRenderer extends RenderingManager {
    ...
    uiLayers.forEach(key => {
      if (!uiLayers.get(key).isHidden())
        uiLayers
          .get(key)
          .getItems()
          .forEach(node => this.renderNode(<CanvasNode>node));
    });
  }
  ...
  protected renderUIElement(uiElement: UIElement): void {
    if (uiElement instanceof Label) {
      ...
      this.textCtx.setTransform(
        1,
        0,
        0,
        1,
        (uiElement.position.x - this.origin.x) * this.zoom,
        (uiElement.position.y - this.origin.y) * this.zoom,
      );
      ...
      this.textCtx.fillText(
        uiElement.text,
        offset.x - uiElement.size.x / 2,
        offset.y - uiElement.size.y / 2,
      );
