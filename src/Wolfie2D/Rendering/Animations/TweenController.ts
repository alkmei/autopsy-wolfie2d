...
export default class TweenController {
  ...
  play(key: string, loop?: boolean): void {
    if (this.tweens.has(key)) {
      ...
      console.warn(
        `Tried to play tween "${key}" on node with id ${this.owner.id}, but no such tween exists`,
      );
    }
  }
    ...
    this.tweens.forEach(key => {
      ...
      if (tween.animationState === AnimationState.PLAYING) {
        ...
        if (tween.elapsedTime >= tween.startDelay) {
          ...
          if (tween.reversing) {
            tween.progress = MathUtils.clamp01(
              (2 * tween.duration - (tween.elapsedTime - tween.startDelay)) /
                tween.duration,
            );
          } else {
            tween.progress = MathUtils.clamp01(
