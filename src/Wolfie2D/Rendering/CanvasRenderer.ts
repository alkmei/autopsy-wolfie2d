...
export default class CanvasRenderer extends RenderingManager {
  ...
  render(
    ...
    while (tilemapIndex < tilemapLength || visibleSetIndex < visibleSetLength) {
      ...
      if (
        tilemaps[tilemapIndex].getLayer().getDepth() <=
    ...
    sortedUILayers = sortedUILayers.sort(
      (ui1, ui2) => ui1.getDepth() - ui2.getDepth(),
    );
  ...
  protected renderNode(node: CanvasNode): void {
    ...
    this.ctx.setTransform(
      ...
      0,
      yScale,
      (node.position.x - this.origin.x) * this.zoom,
      (node.position.y - this.origin.y) * this.zoom,
    );
  ...
  protected renderSprite(sprite: Sprite): void {
    ...
    this.ctx.drawImage(
      image,
      sprite.imageOffset.x,
      sprite.imageOffset.y,
      sprite.size.x,
      sprite.size.y,
      ((-sprite.size.x * sprite.scale.x) / 2) * this.zoom,
      ((-sprite.size.y * sprite.scale.y) / 2) * this.zoom,
      sprite.size.x * sprite.scale.x * this.zoom,
      sprite.size.y * sprite.scale.y * this.zoom,
    );
  }
  ...
  protected renderAnimatedSprite(sprite: AnimatedSprite): void {
    ...
    this.ctx.drawImage(
      image,
      sprite.imageOffset.x + animationOffset.x,
      sprite.imageOffset.y + animationOffset.y,
      sprite.size.x,
      sprite.size.y,
      ((-sprite.size.x * sprite.scale.x) / 2) * this.zoom,
      ((-sprite.size.y * sprite.scale.y) / 2) * this.zoom,
