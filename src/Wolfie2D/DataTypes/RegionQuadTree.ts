...
export default class QuadTree<T extends Region & Unique> implements Collection {
   ...
   * @param uniqueMap A map that stores the unique ids of the results so we know what was already found
   */
  protected _queryRegion(
    boundary: AABB,
    results: Array<T>,
    uniqueMap: Array<boolean>,
  ): void {
    // Does this quadtree even contain the point?
  ...
  public render_demo(
    ...
    ctx.strokeRect(
      (this.boundary.x - this.boundary.hw - origin.x) * zoom,
      (this.boundary.y - this.boundary.hh - origin.y) * zoom,
      2 * this.boundary.hw * zoom,
      2 * this.boundary.hh * zoom,
    );
