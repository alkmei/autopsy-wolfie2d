...
export default class GameLevel extends Scene {
    ...
    startScene(): void {
        ...
        Input.disableInput();
        this.emitter.fireEvent(HW5_Events.SUIT_COLOR_CHANGE, {
            color: HW5_Color.RED,
        });
    }
    ...
    updateScene(deltaT: number) {
        ...
        while (this.receiver.hasNextEvent()) {
            ...
            switch (event.type) {
                case HW5_Events.PLAYER_HIT_SWITCH:
                    {
                        ...
                        this.switchLabel.text =
                            "Switches Left: " +
                            (this.totalSwitches - this.switchesPressed);
                ...
                case HW5_Events.PLAYER_HIT_BALLOON:
                    {
                        let node = this.sceneGraph.getNode(
                            event.data.get("node"),
                        );
                        let other = this.sceneGraph.getNode(
                            event.data.get("other"),
                        );

                        if (node === this.player) {
                            // Node is player, other is balloon
                            this.handlePlayerBalloonCollision(
                                <AnimatedSprite>node,
                ...
                case HW5_Events.BALLOON_POPPED:
                    {
                        ...
                        this.balloonsPopped++;
                        this.balloonLabel.text =
                            "Balloons Left: " +
                            (this.totalBalloons - this.balloonsPopped);
                        let node = this.sceneGraph.getNode(
                        ...
                        let particleMass = 0;
                        if (
                            (<BalloonController>node._ai).color == HW5_Color.RED
                        ) {
                            particleMass = 1;
                        } else if (
                            (<BalloonController>node._ai).color ==
                            HW5_Color.GREEN
                        ) {
                            particleMass = 2;
                        } else {
                ...
                case HW5_Events.PLAYER_ENTERED_LEVEL_END:
                    {
                        ...
                        if (
                            this.switchesPressed >= this.totalSwitches &&
                            this.balloonsPopped >= this.totalBalloons
                        ) {
                            if (
        ...
        if (this.suitChangeTimer.isStopped()) {
            ...
            if (Input.isKeyJustPressed("2")) {
                this.emitter.fireEvent(HW5_Events.SUIT_COLOR_CHANGE, {
                    color: HW5_Color.BLUE,
                });
                this.suitChangeTimer.start();
            }
            if (Input.isKeyJustPressed("3")) {
                this.emitter.fireEvent(HW5_Events.SUIT_COLOR_CHANGE, {
                    color: HW5_Color.GREEN,
                });
                this.suitChangeTimer.start();
            }
        }
    ...
    protected addUI() {
        // In-game labels
        this.balloonLabel = <Label>this.add.uiElement(
            UIElementType.LABEL,
            "UI",
            {
        ...
        this.switchLabel = <Label>this.add.uiElement(
            UIElementType.LABEL,
            "UI",
            {
        ...
        this.system = new HW5_ParticleSystem(
            100,
            new Vec2(5 * 32, 10 * 32),
            2000,
            3,
    ...
    protected initPlayer(): void {
        ...
        if (!this.playerSpawn) {
            console.warn(
                "Player spawn was never set - setting spawn to (0, 0)",
            );
    ...
    protected addLevelEnd(startingTile: Vec2, size: Vec2): void {
        this.levelEndArea = <Rect>this.add.graphic(
            GraphicType.RECT,
            "primary",
            {
                position: startingTile.scale(32),
                size: size.scale(32),
            },
        );
    ...
    */
     ...
     *
     */
    protected handlePlayerBalloonCollision(
        player: AnimatedSprite,
        balloon: AnimatedSprite,
    ) {
        if (balloon == null) return;
        const balloonColor = (<BalloonController>balloon.ai).color;
        const suitColor = (<PlayerController>player.ai).suitColor;
        if (suitColor != balloonColor) this.incPlayerLife(-1);
