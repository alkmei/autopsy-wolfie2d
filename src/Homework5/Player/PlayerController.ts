...
export default class PlayerController extends StateMachineAI {
    ...
    changeState(stateName: string): void {
        ...
        if (
            (stateName === PlayerStates.JUMP ||
                stateName === PlayerStates.FALL) &&
    ...
    update(deltaT: number): void {
        ...
        const colRow = this.tilemap.getColRowAt(
            new Vec2(
                this.owner.collisionShape.x,
                this.owner.collisionShape.y + this.owner.collisionShape.hh + 16,
            ),
        );

        const tile = this.tilemap.getTileAtRowCol(colRow);

        if (this.owner.onGround && tile == 8) {
